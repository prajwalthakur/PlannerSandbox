...
from mppi_planner.critics import GoalCritic, GoalAngleCritic, ObstaclesCritic, PreferForwardCritic, PathAngleCritic
class MPPI:
    def __init__(self,cfg, mppi_key):
        ...
        # Critics
        # -----------------------
        self._load_critics(cfg["critics"])
    ...
    # Critics
    # =====================================================
    def _load_critics(self, critics_cfg):
        ...
        self.obstacle_critic = ObstaclesCritic(critics_cfg["ObstaclesCritic"],self.dim_euclid)
    ...
    def prepare(self,rbt_curr_st,rbt_curr_vel,goal_pose,pred_obs_positions,pred_obs_vels):
        ...
        self.pred_obs_positions = jnp.asarray(pred_obs_positions.reshape((self.num_obs,2)))
        self.pred_obs_vels = jnp.asarray(pred_obs_vels.reshape((self.num_obs,2)))
        
    @functools.partial(jax.jit, static_argnums=0)
    def solve(self,key,temperature,rbt_curr_pose,rbt_curr_vel,prev_control_sequence,rbt_goal_pose,pred_obs_positions,pred_obs_vels):
        ...
        key,nominal_mppi_cost , perturbed_control , delta_u, mppi_trajs = self.cal_nominal_mppi_cost(key,rbt_curr_pose,rbt_curr_vel,rbt_goal_pose,prev_control_sequence, pred_obs_positions, pred_obs_vels,delta_u)
    ...
    def compute_control(self,rbt_curr_st,rbt_curr_vel,goal_pose,pred_obs_poses,pred_obs_vels):
        ...
        self.key, eta, self.u_seqs, mppi_trajs = self.solve(self.key,self.temperature,self.rbt_curr_pose,self.rbt_curr_vel,self.prev_control_sequence,self.rbt_goal_pose,self.pred_obs_positions,self.pred_obs_vels)
    ...
    def batched_obstacle_step(self, pos, vel, obs_noise, dt):
        return pos + (vel + obs_noise) * dt    
    
    
    def make_step_fn(self,predicted_obs_vel,rbt_goal_pose):
        def step_function(carry,inputs):
            rbt_state,obstacles_state,cost,goal_flag = carry
            u_t,obs_noise_t = inputs
            ...
            obstacles_state = self.batched_obstacle_step(obstacles_state,predicted_obs_vel,obs_noise_t,self.ctrlTs)

            #cost 
            stage_cost, goal_flag = self.batched_stage_cost(rbt_state,u_t, goal_flag, obstacles_state, rbt_goal_pose)
            cost = stage_cost+cost

            return (rbt_state,obstacles_state,cost,goal_flag),rbt_state
        return step_function




    def cal_nominal_mppi_cost(self,key,rbt_curr_pose,rbt_curr_vel,rbt_goal_pose,prev_control_sequence,predicted_obs_state, predicted_obs_vel_array, delta_u):
        ...
        :param predicted_obs_state: Description
        ...
        init_carry = (rbt_state,predicted_obs_state,cost0,goal_flag)
        inputs = (u_cliped_seqs.transpose(1,0,2),obs_modelled_noise)
        ...
        rbt_state,obstacles_state,cost,goal_flag = carry_final
    ...
    #equation 3,4,5,6 
    def batched_stage_cost(self, rbt_state, rbt_ctrl, goal_flag, obstacles_state, rbt_goal_pose):       
        """
        ...
        :param obstacles_state: Description (Nox2)
        :param goal_pose: Description (rbt_dim,1)
        """
        # eulcid distance to the obstacle
        cost_obs = self.obstacle_critic(rbt_state,obstacles_state,rbt_goal_pose)
